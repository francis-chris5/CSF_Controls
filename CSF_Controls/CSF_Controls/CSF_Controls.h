...
namespace Sensors{
	 ...
	 * Here is the prototype of the Rotary-Pot-Module to go with classes in this library.  I wasn't about to pay a fortune in shipping for ten of these so I have to wait a month on the PCB's to arrive from China, so it'll be a while before pictures can be taken of the final versions. The thought of smaller and smaller electronics is nice, but I like the size of these controls with my chubby-stubby-northern-european-ancestory fingers when mocking up ideas so the Pot-Module-R Gerber files don't include components, just holes for them to go in and I plan to solder the full size components in myself, but might as well share those here too, if you want to update a copy of the gerber file most any factory wouldn't have any trouble stamping these parts on there for you.\n
